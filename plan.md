...
3. **History** — three filter modes plus past-day logging:
   ...
   - **Week** (`/history/week/[date]`) — the ISO week (Monday → Sunday) containing the URL date. Same DailyTotals chrome as Today, but with **weekly totals** summed across the seven days and **weekly goals** = the most-recent daily-goal snapshot × 7. Below the rollup is a clickable per-day strip (date, kcal, on-target pill, UL dot when applicable). Non-Monday URLs redirect to the canonical Monday. Future weeks 404. Current week clamps the data range to today so future days don't dilute the totals. UL handling deliberately skipped — Tolerable Upper Intake Levels are *daily* safety thresholds, so the per-day dots are the right surface for that signal.
   - **Month** (`/history/month/[month]`) — calendar heatmap colored by % of calorie target, tap a day to drill in.
   - **Range** (`/history/range?from=&to=`) — built but **hidden** from primary navigation. Quick-pick buttons (7 / 30 days, This / Last month) plus custom date inputs, scorecard, per-nutrient sparkline grid (calories, protein, carbs, fat, fiber, sodium — hand-rolled SVG), clickable per-day breakdown. Hidden because the Day / Week / Month entry points cover the most common questions; we may resurface it later for ad-hoc analysis.
